...
import GoogleProvider from 'next-auth/providers/google'; // Added Google Provider
  ...
  callbacks: {
     ...
     async jwt({ token, user, account, profile, isNewUser }): Promise<JWT> {
        if (user) { 
            ...
            // For OAuth users, some fields might not immediately exist, so we only append custom flags if present.
            if ('onboardingComplete' in user) {
